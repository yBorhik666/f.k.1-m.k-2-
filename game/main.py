...
from pause_menu import run_pause_menu, save_game, load_game
...
def run_game(screen):
    ...
    def cast_walls(v_offset=0):
        ...
        for ray in range(NUM_RAYS):
            ...
            if hit:
                ...
                depth = max(depth, 0.1)
                proj_h = 21000 / depth
                tex      = door_tex if hit_door else wall_tex
                tex_w    = tex.get_width()
                tex_h    = tex.get_height()
                tex_x    = int(tex_x_coord * tex_w / TILE)
                tex_x    = max(0, min(tex_x, tex_w - 1))   # не выходим за правый край
    ...
    def check_lilith_interact(keys):
        # ФИЧ БАГА 3+4: keys передаётся как аргумент, функция вызывается только один раз
        nonlocal spear_unlocked, spear_anim_timer
        if lilith is None or not lilith["alive"]:
            return
        dist = math.hypot(px - lilith["x"], py - lilith["y"])
        if dist < 180 and not spear_unlocked and keys[pygame.K_e]:
            spear_unlocked = True
            spear_anim_timer = 0
            ...
            if -fov/2 < angle_to < fov/2 and dist < MAX_DEPTH:
                ...
                if dist < depths[ray]:
                    ...
                    if size >= 20:
                        ...
                        screen.blit(lb, (sx+size//2-lb.get_width()//2, sy+size//2-lb.get_height()//2))
    ...
    def update_bullets():
        ...
        for bullet in bullets[:]:
            ...
            for enemy in enemies:
                ...
                hit_r = 35 if bullet.get("is_spear") else 20
                if math.hypot(enemy["x"]-bullet["x"], enemy["y"]-bullet["y"]) < hit_r:
    ...
    def draw_heat_bar():
        ...
        if overheated:
            ...
            # БАग 2 ИСПРАВЛЕН: было `os = pygame.Surface(...)` — затирало модуль os
            overheat_surf = pygame.Surface((bar_w, bar_h), pygame.SRCALPHA)
    ...
    def draw_minimap():
        ...
        for e in enemies:
            if e["alive"]:
                pygame.draw.circle(surf,(255,255,0),(int(e["x"]//TILE*scale),int(e["y"]//TILE*scale)),3)
        screen.blit(surf,(20,20))

    def draw_level_stats(completed_level):
        pygame.mouse.set_visible(True); pygame.event.set_grab(False)
        s = stat_time_frames // 60
        time_str = f"{s//60:02d}:{s%60:02d}"
        ft = pygame.font.SysFont("impact",72)
        fb = pygame.font.SysFont("impact",42)
        fh = pygame.font.SysFont("impact",24)
        rank, rank_color = (
            ("S",(255,215,0))   if stat_kills>=20 else
            ("A",(100,220,100)) if stat_kills>=12 else
            ("B",(100,160,255)) if stat_kills>=6  else
            ("C",(200,200,200)) if stat_kills>=2  else
            ("D",(160,80,80))
        )
        ...
        while waiting:
            ...
            title = ft.render(f"УРОВЕНЬ {completed_level} — ПРОЙДЕН",True,(220,30,30))
            ...
            for i,(lbl,val,col) in enumerate([
                ...
                ("НАНЕСЕНО УРОНА",str(stat_damage),  (255,160,30)),
            ]):
            ...
            if anim_timer >= 80:
                ...
                if rs > 10:
                    ...
                    rl = fh.render("РЕЙТИНГ",True,(160,160,160))
                    screen.blit(rl, rl.get_rect(center=(WIDTH//2,ry-16)))
            if anim_timer > 60:
                hint = fh.render("[ ENTER / ЛКМ ] — следующий уровень",True,(140,140,140))
    ...
    def draw_death_screen():
        ...
        ht = fs.render("R — продолжить  |  ESC — выход",True,(200,200,200))
        screen.blit(dt, dt.get_rect(center=(WIDTH//2,HEIGHT//2-60)))
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_ESCAPE:
                    pygame.mouse.set_visible(True); pygame.event.set_grab(False)
                    action = run_pause_menu(screen, settings)
                    mouse_sens = settings["mouse_sens"]
                    pygame.mixer.music.set_volume(settings["volume"])
                    if action == "load":
                        state = settings.get("loaded_state")
                        if state:
                            player_hp      = state.get("player_hp", 100)
                            LEVEL          = state.get("level", 1)
                            ammo["9mm"]    = state.get("ammo_9mm", 60)
                            ammo["shells"] = state.get("ammo_shells", 20)
                            ammo["762"]    = state.get("ammo_762", 120)
                            ammo["cells"]  = state.get("ammo_cells", 10)
                            load_level()
                ...
                if event.key == pygame.K_F5:
                    save_game({"player_hp": player_hp, "level": LEVEL,
                               "ammo_9mm": ammo["9mm"], "ammo_shells": ammo["shells"],
                               "ammo_762": ammo["762"], "ammo_cells": ammo["cells"]})
        ...
        # БАГИ 1 ИСПРАВЛЕН: убран мёртвый цикл-заглушка for move_cos, move_sin, sign in [...]
        ...
        # БАГИ 3+4 ИСПРАВЛЕНЫ: один вызов после объявления keys, keys передаётся аргументом
        check_lilith_interact(keys)
        ...
        if is_lilit:
            ...
            current_sky = (min(255, int(sky_color[0] * (0.6 + 0.4 * pulse))), sky_color[1], sky_color[2])
        else:
        ...
        if no_ammo_timer > 0:
            no_ammo_timer -= 1
            naf = pygame.font.SysFont("impact", 36)
            nas = naf.render("НЕТ ПАТРОНОВ!", True, (255,60,60))
            nas.set_alpha(min(255, no_ammo_timer * 6))
            screen.blit(nas, (WIDTH//2 - nas.get_width()//2, HEIGHT//2 - 80))
        ...
        if is_lilit:
            vs = 120
            pa = int(80 + 60 * math.sin(lilit_pulse_timer * 0.7))
            for rx, ry, rw, rh in [
                (0,        0,       vs,     HEIGHT),
                (WIDTH-vs, 0,       vs,     HEIGHT),
                (0,        0,       WIDTH,  vs),
                (0,        HEIGHT-vs, WIDTH, vs),
            ]:
                v = pygame.Surface((rw, rh), pygame.SRCALPHA)
                v.fill((120, 0, 0, pa))
                screen.blit(v, (rx, ry))
...
if __name__ == "__main__":
    ...
    pygame.display.set_caption("not DOOM THE KILL FYRRE")
    while True:
        if run_menu(screen): run_game(screen)
        else: break